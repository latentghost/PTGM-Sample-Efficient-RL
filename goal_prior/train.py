...
from hierarchical import HighlevelAgent, configure_optimizers # goal prior model is the same to the high-level policy
from steve1.data.minecraft_dataset import load_sampling
from goal_prior.dataset import MinecraftGoalPriorDataset
...
class DDPPolicy(torch.nn.Module):
    ...
    def forward(self, obs, agent_state, firsts, actions, action_space_size=(8641, 121)):
        ...
        if random.random() < self.epsilon:
            ...
            actions, new_agent_state, res = self.policy.act_batch(obs, firsts, agent_state, stochastic=False, taken_action=selected_actions, return_pd=True)
            pi_distribution = res['pd']
        
        else:
            # # Select action with the highest probability
            # selected_actions = pi_distribution.argmax(dim=-1)
...
def main(args):
    ...
    # load codebook
    codebook = KMeansCodebook(path=args.codebook_path)
    print('Load codebook, size ', codebook.N)
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers("goal_prior_eps1",
                                init_kwargs={
                                    "wandb": {
                                        "resume": "allow",
                                        "id": wandb_id,
    ...
    dataset = MinecraftGoalPriorDataset(train_episodes, 
                               args.T, 
                               args.min_btwn_goals, 
                               args.max_btwn_goals, 
                               codebook=codebook)
    ...
    val_dataset = MinecraftGoalPriorDataset(val_episodes, 
                                   args.T, 
                                   args.min_btwn_goals, 
                                   args.max_btwn_goals, 
    ...
    agent_action_space = TensorType(shape=(1,), eltype=Discrete(codebook.N))

    #env = gym.make("MineRLBasaltFindCave-v0")
    agent = HighlevelAgent(device=device, action_space=agent_action_space, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--codebook_path', type=str, default='downloads/codebook/tsne_n100_s1/centers.pkl')
    parser.add_argument('--in_model', type=str, default='downloads/2x.model')
    parser.add_argument('--in_weights', type=str, default='downloads/rl-from-foundation-2x.weights')

    parser.add_argument('--out_weights', type=str,
                        default='results/goal_prior/weights/trained_with_script.weights')
    ...
    parser.add_argument('--trunc_t', type=int, default=64)
    ...
    parser.add_argument('--checkpoint_dir', type=str,
                        default='results/goal_prior/training_checkpoint')
