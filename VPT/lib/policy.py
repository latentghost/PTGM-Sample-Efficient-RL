...
class MinecraftAgentPolicy(nn.Module):
    ...
    # def act_batch(self, obs, first, state_in, stochastic: bool = True, taken_action=None, return_pd=False):

    def act_batch(self, obs, firsts, state_in, stochastic=True, taken_action=None, return_pd=False):
        (pd, vpred, _), state_out = self(obs=obs, first=firsts, state_in=state_in)
